...
void abs_path_loop_where(char **path_split, char *bin, char **cmd, char *path)
{
...
int get_abs_path_where(char **envp, char **cmd)
{
    ...
    if (cmd[0][0] != '/' && my_strncmp(cmd[0], "./", 2) != 0)
        abs_path_loop_where(path_split, bin, cmd, path);
...
int error_handling_where(char **array)
{
...
int exec_where(all_t *s_all, char **array, char **envp)
{
    ...
    if (error_handling_where(array) == 1) {
        s_all->the_return = 1;
        return 1;
    }
    for (int i = 1; array[i] != NULL; i++) {
        ...
        if (get_abs_path_where(envp, tmp) == 0 && tmp[0] != NULL)
            my_printf("%s\n%s\n", tmp[0], &tmp[0][4]);
